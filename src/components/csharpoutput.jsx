...
import { executeCode } from "../api"; // Ensure this function is correctly implemented

const Csharpoutput = ({ editorRef }) => {
  ...
  const runCode = async () => {
    ...
    if (!sourceCode) {
      toast({
        ...
        description: "Please enter some C# code before running.",
        status: "warning",
        duration: 4000,
      });
      return;
    ...
    try {
      ...
      const result = await executeCode("csharp", sourceCode);
      console.log("Execution Result:", result); // Debugging
